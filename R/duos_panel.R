#' Plot Traceplots
#'
#' Plots the traceplots of the parameters from \code{duos}.
#'
#'
...
#' @param plots An option to plot parameters on the same plot or as individuals (see details).
...
#'
#' There are two sets of parameters that can plotted in the traceplots: the cut-points and the proportion parameters.
#' \itemize{
#'     \item \code{"c"}: Plots the traceplots of the cut-points that are ordered and between 0 and 1 (DEFAULT).
#'     \item \code{"p"}: Plots the traceplots of the proportion parameters that sum to one.
#'   }
#'
...
#'     \item \code{"indiv"}: Create a grid of traceplots where each plot contains a single parameter's traceplot. Four plots are included in a traceplot so multiple panels are created there are more than 4 parameters.
#'   }
#'
#'
...
#'
#' #Plot the individual traceplots for the proportion parameters
...
duos_panel <- function(duos_output, parameters = "c"){
  ...
  C$Iteration <- burnin:nrow(C_output)
    ...
    for(i in 1:ncol(C)){
      get_acf <- acf(C[,i], plot = FALSE, lag = 100)
      acf_plot_add <- data.frame(rep(i, 101), with(get_acf, data.frame(lag, acf)))
    ...
    acf_plot$Parameter <- factor(acf_plot$Parameter, levels = c(1:ncol(C)))
    
    g_acf <- ggplot(data = acf_plot, mapping = aes(x = Lag, y = ACF)) +
      geom_hline(aes(yintercept = 0)) +
      geom_segment(mapping = aes(xend = Lag, yend = 0, color = Parameter, size = Parameter))+theme_bw()
    g_acf
