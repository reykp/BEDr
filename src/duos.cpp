...
// [[Rcpp::export]]
List duos(NumericVector y, double k=NA_REAL, double MH_N=20000, double alpha=1, double scale_l=0.00001, double scale_u=0.00001, NumericVector start = NA_REAL){
  ...
  if(MH_N<=0){
    throw std::range_error("MH_N should be an integer greater than zero.");
  }
 ...
 // Create initial p vector by simulating from a gamma distribution
  ...
  for(int i=1; i<MH_N;i++){
    ...
    for(int j=0; j<k; j++){
      ...
      for(int l=0; l<k+1; l++){
        temp(i,l) = sum((y_analyize>=c_init_full[l]) & (y_analyize<c_init_full[l+1]));
